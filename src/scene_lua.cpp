...
// Keeps track of the unique node id counter
int isz = 0;
...
int gr_node_add_child_cmd(lua_State* L)
{
  ...
  // Import the GeometryNodes into my special data structure for easy processing
  if ( dynamic_cast<GeometryNode*>(child) )
  {
	  child->set_id( isz );
	  isz++;
	  GeomNode *t = new GeomNode( (GeometryNode*)child, (JointNode*)self );
	  gldata.push_back( t );
  }
